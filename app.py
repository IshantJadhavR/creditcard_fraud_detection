...
from sklearn.metrics import (
    f1_score, precision_recall_curve, auc, roc_curve,
...
st.markdown("""
**Features:**  
- Batch metrics for entire CSV  
- Predict a specific row by index  
- Predict a random transaction from CSV  
...
mode = st.radio(
    "Select operation:",
    ["Batch metrics", "Pick a row", "Random transaction"],
    horizontal=True
)
...
if uploaded_file is not None:
    try:
        ...
        df_for_metrics = df_raw.copy()
        df_model = prepare_features(df_raw)
        ...
        # --- Pick a specific row ---
        elif mode == "Pick a row":
            st.subheader("Predict a Specific Row")
            row_idx = st.number_input("Enter row index (0-based):", min_value=0, max_value=len(df_model)-1, value=0)
            if st.button("Predict Selected Row"):
                ...
                st.success(f"Row {row_idx} Prediction: {'Fraud' if pred_row==1 else 'Legit'} "
                           f"(Fraud Probability: {prob_row:.2%})")

        # --- Random transaction ---
        elif mode == "Random transaction":
            st.subheader("Predict a Random Transaction")
            if st.button("Pick and Predict Random Row"):
                ...
                st.success(f"Random Row Prediction: {'Fraud' if pred_rand==1 else 'Legit'} "
                           f"(Fraud Probability: {prob_rand:.2%})")
